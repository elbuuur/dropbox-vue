...
const fileRetentionDays = ref<number>(30);
...
async function uploadFiles() {
  try {
    ...
    const httpData = await collectAndValidateHttpData(selectedFiles);
...
async function collectAndValidateHttpData(selectedFiles: FileList) {
  try {
    ...
    if (hasRetentionDays.value) {
      await validateFields({
        shelf_life: fileRetentionDays.value,
      });
...
const handleFileChange = () => {
  if (filesInputRef.value && filesInputRef.value.files.length > 0) {
    errors.value = {};
  }
};
</script>
